...
class dbaas_trove::trove {
  ...
  if ($trove_enabled) {
    ...
    $use_nova_server_config_drive  = $trove['use_nova_server_config_drive']
    ...
    class { '::trove':
      ...
      rabbit_userid                => $trove['rabbit_user'],
      rabbit_use_ssl               => false, # Disable rabbit connection over ssl from trove services.
    ...
    class { '::trove::guestagent':
      ...
      rabbit_port    => $amqp_port,
      rabbit_use_ssl => $rabbit_public_ssl,
      auth_url       => false,
      swift_url      => false,
    }
    ...
    class { '::trove::config':
      trove_config            => {
        ...
        'DEFAULT/use_nova_server_config_drive' => { value        => $use_nova_server_config_drive },
      },
